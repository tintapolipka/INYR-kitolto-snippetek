...
if(!vezeteknev){alert('Előbb be kell tölteni adatot az [ADATBETOLTO]-vel, csak utána lehet űrlapot kitölteni! Használd az [ADATBETOLTO]-t!')};
   ...
   function urlapAzonosito(){
    if(document.getElementsByTagName('h1').length>0){
       ...
       case 'Törzslap felvétele':
          ...
          case 'Törzslap ':
          if(document.querySelector('.modal-title').textContent==='Új ellátás hozzáadása' && (document.getElementsByTagName('h3')[0]))
 ...
 function UEhozzadasa(){  
  ...
  if(osztalyfok==='ovodába járó'){document.getElementById('IntezmenyTipus').value = 'OVODA'} else if (osztalyfok<9) {
    ...
    else if(hanyadikVizsg==='fv') { 
       alert('A felülvizsgálat tanéve kézzel kitöltendő! (Nincs róla adat!)')
    }
 }
...
function kitolt(gyp_1_VAGY_pszicho_2){ 
 ...
 document.getElementById('Kezdete').value=vizsgDatum;
 ...
 if(gyp_1_VAGY_pszicho_2===2){document.querySelector(`input[value="${pszichologus}"]`).click()};
 if(gyp_1_VAGY_pszicho_2===3){document.querySelector(`input[value="${megfigyelesVoltE}"]`).click()};
 document.getElementById('TevekenysegSzakemberek').nextElementSibling.click();
...
if(gyp_1_VAGY_pszicho_2==3){localStorage.mefigyMegNemVolt = 'hamis';}
 ...
 function egybekitolt(){
    megnyit();
 if(localStorage.pszichoMegNemVolt === 'igaz'){console.log('van pszichológus, felvisszük!'); setTimeout(()=>{kitolt(2)},1500); }
