...
let [vezeteknev, utonev,vizsgDatum, diagnozis, osztalyfok, hanyadikVizsg,megfigyelesVoltE, gyogyPed,pszichologus,OA, KerelmezoFehId,beErkezett, mikorFigyelteMeg,fvTaneve] = JSON.parse(localStorage['vizsgaltGyerek']);
if(!vezeteknev){alert('Előbb be kell tölteni adatot az [ADATBETOLTO]-vel, csak utána lehet űrlapot kitölteni! Használd az [ADATBETOLTO]-t!')};
 ...
 function ellatottKeresese(){
    ...
    document.querySelector('i[class="fa fa-search"]').parentElement.click();
 }
...
function lezaras(){
   if(document.querySelector('i[class="fa fa-openid"]')){alert('Le van zárva az ellátás!')}
