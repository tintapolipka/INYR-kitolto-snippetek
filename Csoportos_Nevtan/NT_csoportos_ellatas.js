...
//raw data:
let csoportNev, csoportURL, oktataisInt, ellatasNapja, oraHossza,fejlTerv;
...
function ujCsoportFelvetele() {
  ...
  let toAppendString = `<dialog class="inputUjcsopi" open>
        <style>#Zoli-container{ min-height: 50vh;}#Zoli-csoport-container{ min-height: 40vh;}</style>
        ...
        <div id="Zoli-fejlesztesi-terv"> Random generált fejlesztési terv:
                <p>Véletlenszerű gyógypedagógiai fejlesztési terv létrehozása a tárolt sablonokból:</p>
            <div>
                <label>Gyerekek kora: <input name="csoport-intezmenye" type="radio" id="intezmenye-ovoda" checked> Óvodás
            <input name="csoport-intezmenye" type="radio" id="intezmenye-iskola"> Iskolás (alsó tagozat)</div>
            </label>
            <p> Megadok saját fejlesztési tervet: <button onclick="document.getElementById('Zoli-sajat-fejlesztesi-terv').showModal(); document.getElementById('sajat-fejlesztesi-terv').value = fejlTerv.join('; ');">Terv beírása:</button> </p>
            ...
            <textarea name="sajat-fejlesztesi-terv" id="sajat-fejlesztesi-terv" cols="30" rows="10" placeholder="Ide írd /másold be a fejlesztési tervet, az órák anyagát pontosvesszővel (;-vel) elválasztva. pl.: Mintázatfelismerés fejlesztése; Grafomotorika fejlesztése; stb;"></textarea>
                <br>
                <button onclick="fejlTerv = fejlTervBonto(); document.getElementById('Zoli-sajat-fejlesztesi-terv').close()">Mentés</button>
...
function csoportMegnyito(URL) {
  [csoportNev, csoportURL, szakember, oktataisInt, ellatasNapja, oraHossza,fejlTerv] =
