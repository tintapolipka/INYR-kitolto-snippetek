// Variables
    let szakember, oktataisInt, elsoAlkalomDatum, foglDatumok, foglTemak, fejlTeruletek, oraHossza;
    foglDatumok = JSON.parse(localStorage.CS_fogldatumok);
    
// Fő LEFUTTATANDÓ FUNKCIÓ!    
function kezdjukMarEl(){
  [csoportNev,csoportURL,szakember,oktataisInt,ellatasNapja,oraHossza] = JSON.parse(localStorage.NT_aktualis_csoport);
  //Ha nincs meghatározva a szakember, akkor használja a csoport létrehozójának adatát
  if(!szakember){szakemberMeghatarozo();
    function szakemberKiegeszito(){localStorage.setItem('NT_aktualis_csoport',JSON.stringify([csoportNev,csoportURL,szakember,oktataisInt,ellatasNapja,oraHossza]));}
  setTimeout(()=>{szakemberKiegeszito()},850);
  }    
...
setTimeout(()=> {folyamatKoveto()},1000);
...
function tobbSzakemberMeghatarozo(){let kinyertNevek = document.querySelectorAll('div.col-lg-6 div[class="col-lg-6"]')[13].textContent.match(/[A-Z].+/)[0].split(', ');
    ...
    function tevekenysegModalBezaro(){document.querySelector('div.modal-header button').click()}
        ...
        for(let i = 0; i<=kinyertNevek.length;i++){
          if(i<kinyertNevek.length){setTimeout(()=>{szakemberKodKinyero(i)},i*500+300);}
          if(i===kinyertNevek.length){setTimeout(()=>{tevekenysegModalBezaro()},i*500+800);}    
        }
